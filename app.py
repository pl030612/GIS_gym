...
def generate_practice_question_real_data(level: str, qtype: str, unit_id: int | None) -> dict:
    ...
    {{
        ...
        "hint": "給學生的引導提示(Markdown)，例如『考慮使用 st_buffer 函數...』",
...
with tabs[0]:
    with st.container(border=True):
        # [Config] vertical_alignment="bottom" 讓按鈕與選單底部對齊
    ...
    if "pq_data" in st.session_state:
        ...
        # style = q_data.get("style_used", "") # [Removed]
        ...
        if q_hint:
            # [Logic] 使用隱形字元技巧，讓 Streamlit 認為這是新的 Expander 而自動收合
        ...
        # [Config] st.text_area 自動讀取 session_state["p_ans"] 的值
...
if st.session_state["is_ta"] and len(tabs) > 2:
    with tabs[2]:
        ...
        if not df_sub.empty:
            ...
            # [Config] 手動指定欄位順序 (時間 -> 學號 ...)，並隱藏 id/assignment_id
